...
class ViewUser extends React.Component {
    ...
    if (this.state.hasPicture) {
      ...
      await strapi
        ...
        .then(async function(response) {
          ...
          await strapi.axios
            .get(strapiURL + "/userpictures?user=" + userId)
            .then(async function(response2) {
              console.log(response2.data[0]);

              if (response2.data.length === 0) {
                await strapi.axios.post(
                  strapiURL + "/userpictures",
                  {
                    user: userId,
                ...
                await strapi.axios
                  .delete(
                    strapiURL + "/userpictures/" + response2.data[0]._id,
                    {
                      headers: {
                        Authorization: authToken
                      }
  ...
  async componentDidMount() {
    ...
    const pic = await strapi.axios.get(
      strapiURL + "/userpictures?user=" + userId
    );
    console.log(pic);
        ...
        <div>
          ...
          <div>
            <Grid container layout={"row"} justify="center" spacing={24}>
              <Grid item xs={12} md={2}>
                ...
                >
                  <CardHeader title="Profile Picture"></CardHeader>
                  <Divider />
              ...
              <Grid item xs={12} md={8}>
                <Card className={classes.card} style={{ height: "100%" }}>
                  <CardHeader title="Profile Details"></CardHeader>
                  <Divider />
          ...
          <div>
            <Grid container justify="center" spacing={24}>
              <Grid item xs={12} md={10}>
                <Card>
                  <CardHeader title="Associated Capstones" />
                  <Divider />
                  <br />
                </Card>
              </Grid>
              ...
              <Grid container spacing={8}>
                {userCapstones.map(result => (
                  <Grid item xs={12} md={6} style={{ marginTop: "1%" }}>
                    <Card
                      className={classes.associatedCard}
                      onClick={e => ViewUser.handleCapstoneClick(result["_id"])}
                    >
                      <CardContent>
                        <SubHeadingTextTypography
                          text={result["CapstoneName"]}
                        />
                        <Divider />
