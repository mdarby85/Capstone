        ...
        <div>
          ...
          <Grid container justify="center" style={{ marginBottom: "16px" }}>
            <Grid item xs={12} md={10}>
              <GridList
                cellHeight={250}
                cols={ViewSponsors.getColumns(this.props)}
              >
                {match.map((result, i) => (
                  <GridListTile
                    key={strapiURL + result["logo"].url}
                    onClick={e => ViewSponsors.handleTileClick(result.id)}
                  >
                    <img
                      src={strapiURL + result["logo"].url}
                      alt={"Sponsor Logo"}
                    ...
                    <GridListTileBar
                      title={result.name}
                      subtitle={
                        "Sponsors: " + result["capstones"].length + " capstones"
                      }
                      actionIcon={
                        <IconButton
                          className={classes.icon}
                          href={"/ViewASponsor/" + result.id}
                        >
                          <InfoIcon />
                        </IconButton>
                      }
...
export default compose(withStyles(styles), withWidth())(ViewSponsors);
