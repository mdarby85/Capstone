...
class ViewCapstone extends React.Component {
  ...
  handleSubmit(capstoneId) {
    ...
    strapi.axios
      ...
      .then(async function(response) {
        ...
        if (
          typeof image.files[0] !== "undefined" &&
          typeof image.files[0].name !== "undefined"
        ) {
  ...
  handleUserSubmit(capstoneId, creators) {
    ...
    for (let i = 0; i < this.state.users.length; i++) {
      if (
        this.state.users[i].username.toUpperCase() ===
        this.state.newUser.toUpperCase()
      ) {
  ...
  async getTeamPics() {
    ...
    for (let member in this.state.team) {
      pic = await strapi.axios.get(
        strapiURL + "/userpictures?user=" + this.state.team[member]._id
      );
      console.log(pic);
  ...
  async componentDidMount() {
    ...
    if (localStorage.getItem("USER") !== null) {
      ...
      await updateDeptViewCount(
        this.props.match.params.capstoneName,
            ...
            <Grid item xs={10}>
              <Card className={classes.card}>
                <CardContent>
                  ...
                  <PageTitleTypography
                    text={this.state.capstone.CapstoneName}
                  />

                  <Divider />
                  <Typography align={"center"} style={{ marginBottom: "1%" }}>
                    <img
                      src={strapiURL + this.state.capstone["DisplayPhoto"].url}
                      className={classes.capstoneImage}
                      alt="Display"
                    />
                  </Typography>
                </CardContent>
                  ...
                  <Grid item xs={12} sm={12} md={6}>
                    <Card className={this.setLeftColClass(this.props)}>
                      <CardContent>
                        ...
                        >
                          <b>Start Date:</b>{" "}
                          {ViewCapstone.formatDate(
                            this.state.capstone["StartDate"]
                          )}
                        </Typography>
                        <Typography variant="subheading">
                          <b>Date Completed:</b>{" "}
                          {ViewCapstone.formatDate(
                            this.state.capstone["EndDate"]
                          )}
                        </Typography>
                        <Typography
                          variant="subheading"
                        ...
                        >
                          <b>Department: </b>{" "}
                          {this.state.capstone["department"]["name"]}
                        </Typography>
                        <Typography variant="subheading">
                          <div>
                            <b>Sponsors:</b>
                        ...
                        >
                          <b>Description: </b>{" "}
                          {this.state.capstone["Description"]}
                        </Typography>
                      </CardContent>
                    </Card>
                                  ...
                                  <Button
                                    onClick={() =>
                                      this.handlePhotoSubmit(
                                        this.state.capstone["_id"],
                                        this.state.capstone["creators"]
                                      )
                                    }
                                    color="primary"
                                  >
                          ...
                          {this.isCreator(this.state.capstone) && (
                            <div align="center">
                              ...
                              >
                                ...
                                <DialogContent>
                                  <DialogContentText>
                                    To add a new group member, type their
                                    username here. Group members must be
                                    registered on the site to be added to
                                    capstones.
                                  </DialogContentText>
                                  <TextField
                                    margin="dense"
                          ...
                          {creatorArray.length > 0 && (
                            <Grid container style={{ marginTop: "2%" }}>
                              {creatorArray.map((result, i) => (
                                <Grid
                                  item
                                  xs={ViewCapstone.getColumnsForTeamPics(
                                    this.props
                                  )}
                                >
                                  <div className={classes.textContainer}>
                                    <Button
                                      href={"/ViewUser/" + result.username}
                                      style={{ height: "100%", width: "100%" }}
                                    >
                                      <div>
                                        {ViewCapstone.showPicture(
                                          picCreatorArray[i]
                                        )}
                                      </div>
              ...
              <Card className={classes.card}>
                ...
                <div>
                  <Grid
                    container
                    justify={ViewCapstone.alignMakePostButton(this.props)}
                  >
                    <Button
                      variant="outlined"
                      color="primary"
                ...
                <Grid item xs={12} style={{ marginTop: "1%" }}>
                  <Card>
                    {postArray.map((result2, j) => (
                      <Grid xs={12}>
                        <CardActionArea href={"/ViewPost/" + result2.id}>
                          <CardContent>
                            <Typography
                              variant={ViewCapstone.resizePostTitleText(
                                this.props
                              )}
                            >
                              <b>{result2.title}</b>
                            ...
                            <Typography
                              variant={ViewCapstone.resizePostContentText(
                                this.props
                              )}
                            >
                              {result2.content}
                        ...
                        <CardContent>
                          <Grid
                            container
                            justify={ViewCapstone.alignMakePostButton(
                              this.props
                            )}
                          >
                            {this.isPostOwner(result2) && (
                              <Button
                                ...
                                size="small"
                                onClick={e =>
                                  this.handleClickOpen(e, "editOpen", result2)
                                }
                                style={{ marginRight: "2%" }}
                              >
                                Edit
                            ...
                            {this.isPostOwner(result2) && (
                              <Button
                                ...
                                size="small"
                                onClick={e =>
                                  ViewCapstone.handleDelete(e, result2.id)
                                }
                              >
                                Delete
                              </Button>
                          ...
                          >
                            ...
                            <DialogActions>
                              <Button
                                ...
                                disabled={!this.isFormValid()}
                                onClick={e => {
                                  this.handleEdit(e, this.state.postId);
                                }}
                              >
                                Edit Post
                              </Button>
...
export default compose(withStyles(styles), withWidth())(ViewCapstone);
