  ...
  async componentDidMount() {
    ...
    if (tempUser.createdcapstones.length > 0) {
      ...
      for (let i = 0; i < tempUser.createdcapstones.length; i++) {
        let tempCapstone = await strapi.axios.get(
          strapiURL + "/capstones/" + tempUser.createdcapstones[i]["_id"]
        );
        ...
        <div className="Blogpost">
          <Grid container justify="center">
            <Grid item md={10} xs={12}>
              <Typography variant="h4" style={{ marginTop: "16px" }}>
                Your Moderated Capstone Projects
              </Typography>
              <Divider />
              <br />
            </Grid>
          ...
          <Grid container justify="center" style={{ marginBottom: "16px" }}>
            <Grid item xs={12} md={10}>
              <GridList
                cellHeight={250}
                cols={ViewYourCapstones.getColumns(this.props)}
              >
                {this.state.capstones.map((result, i) => (
                  <GridListTile
                    key={strapiURL + result["DisplayPhoto"].url}
                    onClick={e => ViewYourCapstones.handleTileClick(result.id)}
                  >
                    <img
                      src={strapiURL + result["DisplayPhoto"].url}
                      alt={"Capstone"}
                        ...
                        <Grid container>
                          ...
                          <Grid item xs={3} style={{ marginLeft: "2px" }}>
                            <IconButton
                              className={classes.icon}
                              onClick={e =>
                                ViewYourCapstones.handleDelete(e, result.id)
                              }
                              href={"/ViewYourCapstones/"}
                            >
                              <DeleteForeverIcon />
                            </IconButton>
                          </Grid>
          ...
          <Grid container justify="center">
            <Grid item md={10} xs={12}>
              <Typography variant="h4" style={{ marginTop: "16px" }}>
                Your Participated Capstone Projects
              </Typography>
              <Divider />
              <br />
            </Grid>
            ...
            <Grid item xs={12} md={10}>
              ...
              >
                {this.state.participatedCapstones.map((result2, i) => (
                  <GridListTile
                    key={strapiURL + result2["DisplayPhoto"].url}
                    onClick={e =>
                      ViewYourCapstones.handleTileClick(result2._id)
                    }
                  >
                    <img
                      src={strapiURL + result2["DisplayPhoto"].url}
                      alt={"Capstone"}
                      style={{ height: "100%", width: "100%" }}
                    />
                    <GridListTileBar
...
export default compose(withStyles(styles), withWidth())(ViewYourCapstones);
