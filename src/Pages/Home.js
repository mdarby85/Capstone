...
class Home extends React.Component {
  ...
  async componentDidMount() {
    ...
    var advertisement = (
      <div className="col-lg-3 col-md-3">
        <a href={adUrl[1]}>
          <img alt={"Ad Source"} src={adUrl[0]} onClick={adUrl[1]} width={300} height={260} />
        </a>
      </div>
    );
  ...
  top5MostViewedCapstones() {
    ...
    while (i !== n && n > 0) {
      ...
      while (
        j >= 0 &&
        this.state.mostViewedCapstones[j]["viewcount"] < key["viewcount"]
      ) {
        this.state.mostViewedCapstones[j + 1] = this.state.mostViewedCapstones[
          j
        ];
        j = j - 1;
      }
    ...
    this.state.mostViewedCapstones.splice(
      4,
      this.state.mostViewedCapstones.length - 4
    );
  }
      ...
      return (
        <div>
            ...
            <Grid item xs={12} md={4}>
              <CardContent>
                ...
                <Typography variant="body1" style={{ marginTop: "1%" }}>
                  Cappy is a Capstone Management System designed for students
                  and sponsors alike. We provide students a platform to{" "}
                  <b>store, edit, and present</b> their capstone projects.
                </Typography>
              </CardContent>
            </Grid>
          ...
          <Grid container justify="center">
            <Grid item xs={12} md={8}>
              <SubHeadingTextTypography
                ...
                text="Check Out Our Featured Capstones!"
                align="center"
              />
              <Divider variant="middle" />
              <div className={classes.gridListContainer}>
                <GridList
                  className={classes.photoGalleryGridList}
                  cols={this.getColumns(this.props)}
                >
                  {this.state.featuredCapstones.map((result, i) => (
                    <GridListTile
                      ...
                      key={
                        strapiURL +
                        this.state.featuredCapstones[i]["DisplayPhoto"].url
                      }
          ...
          <Grid container justify="center" style={{ marginTop: "1.5%" }}>
            <Grid item xs={12} md={8}>
              <SubHeadingTextTypography
                text={"Sponsored Message from " + this.state.sponsorName + "!"}
                align="center"
              />
              <Divider variant="middle" />
              <div className={classes.gridListContainer}>
