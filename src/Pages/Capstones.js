  ...
  render() {
    ...
    if (!this.state.loading) {
      ...
      let searchOptions = {
        ...
        keys: ["CapstoneName", "department.name", "sponsors.name"]
      };
          ...
          <Grid container justify="center" style={{ marginBottom: "16px" }}>
            <Grid item xs={12} md={10}>
              <GridList cellHeight={250} cols={Capstone.getColumns(this.props)}>
                ...
                {match.map((result, i) => (
                  <GridListTile
                    key={strapiURL + match[i]["DisplayPhoto"].url}
                    onClick={e => Capstone.handleTileClick(result["_id"])}
                  >
                    {/*The display photo for each gridlisttile*/}
                    <img
                      src={strapiURL + match[i]["DisplayPhoto"].url}
                      alt={"Capstone"}
...
export default compose(withStyles(styles), withWidth())(Capstone);
