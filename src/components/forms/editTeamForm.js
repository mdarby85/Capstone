...
export default ({ name, logo, users, id }) => {
    ...
    const [createTeam, { loading: mutationLoading, error: mutationError, data: mutationData }] = useMutation(
        EDIT_TEAM_QUERY
    )
    const { loading: imageLoading, error: imageError, data: imageData } = useQuery(IMAGE_QUERY, {
    ...
    return (
        <form onSubmit={handleSubmit(onSubmit)}>
            ...
            <select name="logo" ref={register()}>
                ...
                {imageData &&
                    imageData.user.mediaLibrary &&
                    imageData.user.mediaLibrary.map(node => {
                        return (
                            <option value={node.id} selected={logo ? logo.id === node.id : false}>
                                {node.name}
                            </option>
                        )
                    })}
            ...
            <select name="members" ref={register()} multiple>
                ...
                {userData &&
                    userData.users.map(node => {
                        return (
                            <option value={node.id} selected={studentOnTeam(node.id)}>
                                {node.name}
                            </option>
                        )
                    })}
