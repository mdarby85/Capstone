...
export default ({ onCreateSuccess }) => {
    ...
    const [createProgram, { loading: mutationLoading, error: mutationError }] = useMutation(CREATE_PROGRAM_MUTATION, {
    ...
    const { data } = useQuery(IMAGE_QUERY, { variables: { id: TEST_USER_ID } })
    ...
    return (
        <div>
            ...
            <form onSubmit={handleSubmit(onSubmit)} name="Edit Program">
                ...
                <select name="thumbnail" ref={register()}>
                    <option disabled selected value="">
                        Select An Image
                    </option>
                    {data &&
                        data.user &&
                        data.user.mediaLibrary.map(img => (
