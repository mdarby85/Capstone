...
export default ({ name, email, roleLabel, archived, id, onEditSuccess }) => {
    ...
    const [createUser, { loading: mutationLoading, error: mutationError, data: mutationData }] = useMutation(
        USER_EDIT_QUERY,
        {
            onCompleted: () => {
                onEditSuccess()
    ...
    return (
        <form onSubmit={handleSubmit(onSubmit)}>
            ...
            <select name="role" ref={register()}>
                {roles.map(r => {
                    return (
                        <option value={r} selected={roleLabel === r}>
                            {r}
                        </option>
                    )
                })}
            ...
            <input
                ...
                ref={register()}
                checked={archivedVal}
                onClick={() => setArchivedVal(!archivedVal)}
            />
