...
export default ({ id, prefix, number, semester, year, name, startDate, endDate, onEditSuccess }) => {
    // Various states of our query fetch
    const { loading, error, data } = useQuery(GET_COURSE_DEPARTMENT)

    // Various states of our mutation
    const [editCourse, { loading: mutationLoading, error: mutationError }] = useMutation(EDIT_COURSE, {
        onCompleted: () => {
            onEditSuccess()
        },
    })
    ...
    return (
        <div>
            ...
            <form onSubmit={handleSubmit(onSubmit)} name="Edit Course">
                ...
                <input name="number" type="text" defaultValue={number} ref={register()} />
