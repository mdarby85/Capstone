...
export async function getAdvertisement() {
          ...
          await strapi.axios
            ...
            .then(async response => {
              ...
              for (var j = 0; j < response.data.length; j++) {
                if (
                  response.data[j].sponsors != null &&
                  response.data[j].sponsors !== []
                ) {
                  for (var k = 0; k < response.data[j].sponsors.length; k++) {
                    if (response.data[j].sponsors[k].isCompany === true) {
                      if (
                        !sponsorOptions.includes(
                          response.data[j].sponsors[k]._id
                        )
                      ) {
              ...
              sponsorId =
                sponsorOptions[
                  Math.floor(Math.random() * sponsorOptions.length)
                ];

              // Step 5:  Now let's get an image URL from the chosen sponsor.
              await strapi.axios
                .get(strapiURL + "/sponsors", {
                  params: {
                    _id: sponsorId
                  }
                })
                .then(async response => {
                  if (response.data.length > 0) {
                    ...
                    if (numAdverts === 0) return await getRandomAd();
                    adLink =
                      strapiURL +
                      response.data[0].adverts[
                        Math.floor(Math.random() * numAdverts)
                      ].url;
                    sponsorUrl = response.data[0].webUrl;
                    sponsorName = response.data[0].name;
                    console.log(
                      "Returning targeted advertisement from " + sponsorName
                    );
                    return [adLink, sponsorUrl, sponsorName];
                  } else return await getRandomAd();
                })
                .catch(async error => {
  ...
  if (sponsorUrl !== null) return [adLink, sponsorUrl, sponsorName];
  else return getRandomAd();
}
...
async function getRandomAd() {
  ...
  await strapi.axios
    ...
    .then(response => {
      ...
      for (var i = 0; i < sponsorArray.length && adUrl === null; i++) {
        if (sponsorArray[i].adverts.length > 0) {
          adUrl =
            strapiURL +
            sponsorArray[i].adverts[
              Math.floor(Math.random() * sponsorArray[i].adverts.length)
            ].url;
    ...
    .catch(error => {
      console.log(
        "We encountered an error while trying to get a random ad link.",
        error
      );
      return [];
    });
        ...
        await strapi.axios
          ...
          .then(async response => {
            if (response.data.length === 0) {
              ...
              await strapi.axios
                .put(strapiURL + "/viewstats/" + viewStatsId, {
                  count: "" + newCount
                })
                .then(response => {
