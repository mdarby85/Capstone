...
import { API_URL, TEST_USER_ID } from "src/constants"
    ...
    return (
        <DashboardLayout>
            ...
            <form onSubmit={onSubmit}>
                <input type="file" name="files" onChange={onImageChange} alt="image" multiple={true} />
            ...
            <div>
                <Row>
                    ...
                    {data &&
                        data.user.mediaLibrary &&
                        data.user.mediaLibrary.map(node => (
                            <div key={node.id}>
                                <Col md={3}>
                                    <p>{node.name}</p>
